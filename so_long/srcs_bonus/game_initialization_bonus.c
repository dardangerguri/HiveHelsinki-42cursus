...
/*   Updated: 2023/09/13 17:06:32 by dgerguri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	assign_image(t_game *game, char character, int x, int y)
{
	if (game->player == 0)
	{
		ft_printf(1, "The player has been killed!\n");
		exit(0);
	}
	...
	if (character == '1')
	...
	else if (character == 'E' && game->components.collectibles != 0)
		...
		mlx_put_image_to_window(game->mlx, game->window, game->exit_o, x, y);
}
...
void	sprite_initialization(t_game *game)
{
	...
	game->exit_o = mlx_xpm_file_to_image(game->mlx, "sprites/ex_o.xpm", &x, &y);
	game->enemy = mlx_xpm_file_to_image(game->mlx, "sprites/enemy.xpm", &x, &y);
}
