...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/21 12:28:53 by dgerguri          #+#    #+#             */
/*   Updated: 2022/12/11 19:49:59 by dgerguri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (buf == NULL)
		return (NULL);
	archive = reading_saving(fd, archive, buf);
	if (archive == NULL)
		return (NULL);
	line = line_from_archive(archive);
	if (line == NULL)
		return (NULL);
	archive = unused_in_archive(archive);
	return (line);
}
